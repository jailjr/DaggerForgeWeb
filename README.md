...
The current deliverable is a polished, runnable frontend and domain seam intended to connect to a free-tier API. For production, place a small TypeScript API in front of the same domain rules, backed by Postgres (Neon/Supabase free tier), signed campaign/GM tokens, participant cookies, row-version optimistic concurrency, append-only audit events, and WebSocket/SSE subscriptions. Store avatars in provider object storage, never in normal rows. The browser demo deliberately uses localStorage so it runs without accounts or cloud credentials.
