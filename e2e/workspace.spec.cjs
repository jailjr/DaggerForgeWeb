...
test('GM can create a live campaign and open the character workflow', async ({ page }) => {
  ...
  const traitList = page.getByRole('list', { name: 'Character traits' });
  await expect(traitList.locator('.trait-row')).toHaveCount(6);
  await expect(page.locator('.traits-grid')).toHaveCount(0);
  await expect(page.getByLabel('Agility modifier')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Mark Agility trait' })).toBeVisible();
  await page.getByRole('button', { name: 'Mark Agility trait' }).click();
  await expect(page.getByRole('button', { name: 'Unmark Agility trait' })).toBeVisible();
  await expect(page.locator('.derived-grid')).toContainText('11');
  await expect(page.locator('.derived-grid')).toContainText('6');
  await page.reload();
  await page.getByRole('button', { name: 'Characters', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Browser Hero' })).toBeVisible();
  await expect(page.locator('.derived-grid')).toContainText('11');
