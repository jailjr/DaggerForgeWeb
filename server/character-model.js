...
const { tierForLevel, normalizeTraitProgression, formatTraitValue } = require('./trait-progression');
...
function canonicalTraitValue(value) { return formatTraitValue(value); }
...
function derivedValuesFor(character) {
  ...
  const progression = normalizeTraitProgression(character);
  const hpMax = selectedClass?.stats?.hp == null ? null : Math.min(12, Number(selectedClass.stats.hp) + ancestryBonuses.hitPoints + subclassBonuses.hitPoints + progression.hpAdvancements);
  const stressMax = Math.min(12, 6 + ancestryBonuses.stress + subclassBonuses.stress + progression.stressAdvancements);
  ...
  return {
    ...
    armorSlots: armor?.slots ?? armor?.armorSlots ?? armor?.score ?? 0,
    armorId: armor?.id || '',
...
function buildCharacterFromChoices(body, id) {
  ...
  const submittedTraits = body.traits && typeof body.traits === 'object' ? Object.fromEntries(traits.map(name => [name, { value: canonicalTraitValue(body.traits[name]?.value ?? body.traits[name] ?? classTraits[traits.indexOf(name)] ?? ''), marked: false }])) : Object.fromEntries(traits.map((name, index) => [name, { value: canonicalTraitValue(classTraits[index] || ''), marked: false }]));
  ...
  return { id, name: String(body.name || 'New Character').slice(0, 120), className: selectedClass?.name || String(body.className || ''), subclassName: String(body.subclassName || ''), ancestry: ancestry?.name || String(body.ancestry || ''), community: community?.name || String(body.community || ''), color: String(body.color || '#a88be3'), initials: String(body.initials || body.name || 'PC').split(/\s+/).map(x => x[0]).join('').slice(0, 4), level, hp: Number.isInteger(Number(body.hp)) ? Math.min(derived.hpMax, Number(body.hp)) : derived.hpMax, hpMax: derived.hpMax, hope: Number.isInteger(Number(body.hope)) ? Math.max(0, Math.min(6, Number(body.hope))) : 2, hopeMax: 6, stress: Number.isInteger(Number(body.stress)) ? Math.max(0, Math.min(derived.stressMax, Number(body.stress))) : 0, stressMax: derived.stressMax, armor: derived.armorScore, armorScore: derived.armorScore, armorSlotsCapacity: derived.armorSlots, armorId: derived.armorId, armorName: derived.armorName, armorFeature: derived.armorFeature, armorSlots: Array.from({ length: derived.armorSlots }, (_, index) => ({ id: index + 1, marked: false })), evasion: derived.evasion, majorThreshold: derived.majorThreshold, severeThreshold: derived.severeThreshold, traits: submittedTraits, traitProgression: { tier: tierForLevel(level), increasedTraits: [], traitAdvancementUses: 0, hpAdvancements: 0, stressAdvancements: 0, history: [], advancements: [] }, experiences: submittedExperiences, domainCards, ancestryCard: ancestry || null, communityCard: community || null, primaryWeapon: weaponRecord(primary, 'Primary'), secondaryWeapon: weaponRecord(secondary, 'Secondary'), complete: body.complete === true, ancestryFeatures: ancestry?.features || [], communityFeatures: community?.features || [], classFeatures: [...(selectedClass?.classFeatures || []), ...subclassFeatures], classBackgroundQuestions: selectedClass?.backgroundQuestions || [], classConnectionQuestions: selectedClass?.connectionQuestions || [], backgroundAnswers: normalizeAnswers(body.backgroundAnswers), connectionAnswers: normalizeAnswers(body.connectionAnswers), ownerId: null, inventory: submittedInventory, privateNotes: String(body.privateNotes || '').slice(0, 5000), ...specializedFor(body, selectedClass?.name || String(body.className || '')), version: 1, lastFieldVersions: {} };
}
...
function validateCharacterModel(character, changes) {
  ...
  if ('armorSlots' in changes && (!Array.isArray(changes.armorSlots) || changes.armorSlots.length !== Number(merged.armorSlotsCapacity ?? merged.armorScore ?? 0) || changes.armorSlots.some(slot => !slot || !Number.isInteger(Number(slot.id)) || typeof slot.marked !== 'boolean'))) return 'Armor slots must match the currently equipped armor.';
...
function applyDerivedValues(character, changes) {
  ...
  if (armor) { next.armorSlotsCapacity = Number(armor.slots ?? armor.armorSlots ?? armor.score ?? 0); next.armorSlots = Array.from({ length: next.armorSlotsCapacity }, (_, index) => ({ id: index + 1, marked: Boolean(character.armorSlots?.[index]?.marked) })); } else if ('armorName' in changes || 'armorId' in changes) next.armorSlots = [];
...
module.exports = { buildCharacterFromChoices, validateCharacterModel, applyDerivedValues, emptyTraits, derivedValuesFor, tierForLevel, normalizeTraitProgression };
