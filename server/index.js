...
async function route(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && parts[3] === 'characters' && !parts[4]) { if (actor.role !== 'GM') return error(res, 403, 'GM access required'); const body = await readBody(req); const characterId = id(); const name = String(body.name || 'New Character').slice(0, 120); campaign.characters[characterId] = { id: characterId, name, className: String(body.className || ''), ancestry: String(body.ancestry || ''), color: String(body.color || '#a88be3'), initials: String(body.initials || name.split(/\s+/).map(x => x[0]).join('').slice(0, 4) || 'PC'), hp: Number.isFinite(body.hp) ? body.hp : 0, hpMax: Number.isFinite(body.hpMax) ? body.hpMax : 6, hope: Number.isFinite(body.hope) ? body.hope : 0, stress: Number.isFinite(body.stress) ? body.stress : 0, armor: Number.isFinite(body.armor) ? body.armor : 0, level: Number.isFinite(body.level) ? body.level : 1, complete: false, ownerId: null, inventory: Array.isArray(body.inventory) ? body.inventory.slice(0, 50).map(String) : [], privateNotes: '', version: 1 }; audit(campaign, actor, 'character.create', 'character', characterId, null, null, campaign.characters[characterId]); save(); return json(res, 201, publicCharacter(campaign.characters[characterId], true)); }
