...
function Pips({ label, value, max, color, onChange, variant = 'bar' }: { label: string; value: number; max: number; color: string; onChange: (value: number) => void; variant?: 'bar' | 'hope' }) {
  const count = Math.max(0, Number(max) || 0);
  return <div className="resource-track"><div className="resource-track-head"><span>{label}</span><b>{value}<small> / {count}</small></b></div><div className={'resource-pips ' + variant}>{Array.from({ length: count }, (_, index) => { const marked = index < value; return <button type="button" key={index} className={'resource-pip ' + variant + ' ' + (marked ? 'filled' : '')} style={marked ? { '--pip-color': color } as React.CSSProperties : undefined} aria-pressed={marked} aria-label={`${label} ${index + 1} of ${count} — ${marked ? 'marked' : 'unmarked'}`} title={`${label} ${index + 1} of ${count} — ${marked ? 'marked' : 'unmarked'}`} onClick={() => onChange(index + 1 === value ? index : index + 1)} />; })}</div></div>;
}

function Panel({ title, eyebrow, children, className = '', id }: { title: string; eyebrow?: string; children: React.ReactNode; className?: string; id?: string }) {
  return <section id={id} className={`sheet-panel ${className}`}><div className="sheet-panel-title"><div>{eyebrow && <span className="section-label">{eyebrow}</span>}<h3>{title}</h3></div></div>{children}</section>;
}
...
export default function CharacterSheetGameplay({ c, mode, onUpdate, onResourceAction, onAdvance, onOpenConfiguration }: Props) {
  ...
  return <>
    ...
    <div className="sheet-gameplay-grid"><div className="sheet-column">
      <Panel id="damage-health" title="Damage & Health" eyebrow="RESOURCES" className="damage-health-panel"><div className="damage-thresholds"><div><span>Minor</span><strong>1–{Math.max(0, Number(c.majorThreshold || 1) - 1)}</strong></div><div><span>Major</span><strong>{c.majorThreshold ?? '—'}</strong></div><div><span>Severe</span><strong>{c.severeThreshold ?? '—'}</strong></div></div><p className="panel-hint">Mark HP for damage. Stress overflow marks one HP when Stress is full.</p><Pips label="HP" value={Number(c.hp || 0)} max={Number(c.hpMax || 0)} color={c.color} onChange={value => setResource('hp', value)} /><Pips label="Stress" value={Number(c.stress || 0)} max={Number(c.stressMax || 6)} color="#8f7bd0" onChange={value => setResource('stress', value)} /></Panel>
      <Panel title="Hope" eyebrow="RESOURCE" className="hope-panel"><p className="panel-hint">Spend Hope to use an Experience or help an ally.</p><Pips label="Hope" value={Number(c.hope || 0)} max={6} color="#d0a75b" variant="hope" onChange={value => setResource('hope', value)} />{c.hopeFeature && <div className="feature-copy">{c.hopeFeature}</div>}</Panel>
      ...
      <Panel id="active-weapons" title="Active Weapons" eyebrow="LOADOUT" className="weapons-panel">{[c.primaryWeapon, c.secondaryWeapon].filter(Boolean).map((weapon: any, index: number) => { const trait = weapon.trait || String(weapon.traitRange || '').split(' - ')[0] || '—'; const range = weapon.range || String(weapon.traitRange || '').split(' - ')[1] || '—'; return <article className="weapon-card" key={weapon.slot || weapon.name}><div className="weapon-card-icon">{index === 0 ? <Swords size={18} /> : <Sparkles size={18} />}</div><div><span className="section-label">{weapon.slot || (index === 0 ? 'PRIMARY' : 'SECONDARY')}</span><h4>{weapon.name}</h4><p>{trait} · {range} · {weapon.damage || weapon.damageDice || '—'} · {weapon.damageType || '—'}</p><small>{weapon.feature || 'No feature text configured.'}</small></div></article>; })}</Panel>
